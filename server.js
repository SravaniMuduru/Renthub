...
//   host: 'localhost',
//   user: 'root',
//   password: 'Sravani@1710',
//   database: 'newrenthubdb',
//   port: 3306,
...
const pool = mysql
  .createPool({
    ...
    database: process.env.DB_NAME || "newrenthubdb",
...
// Example: Add a new listing with image
app.post("/api/listings", upload.single("image"), async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO listings (title, price, location, image) VALUES (?, ?, ?, ?)",
      [title, price, location, imagePath]
    );
